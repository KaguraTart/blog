...
writeFileSync(join(blogDir, 'new-paper.md'), '---\ntitle: 新文章\ndescription: test\npubDate: 2026-05-25\ntags: []\ncategory: Tech\n---\n\n正文\n');

for (const locale of ['en', 'ja', 'fr']) {
  mkdirSync(join(blogDir, locale), { recursive: true });
  writeFileSync(join(blogDir, locale, 'new-paper.md'), '---\ntitle: translated\ndescription: test\npubDate: 2026-05-25\ntags: []\ncategory: Tech\n---\n\nbody\n');
}

const missing = findMissingTranslations({ rootDir: root, locales: ['en', 'ja', 'fr', 'de'] });

assert.deepEqual(missing, [
  { source: 'new-paper.md', locale: 'de', expected: 'src/content/blog/de/new-paper.md' },
]);

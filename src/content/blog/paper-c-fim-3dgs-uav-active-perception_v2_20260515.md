...
description: "v1 定位 RA-L 偏快速发表，老师要求顶刊。本文将 FIM-3DGS 工作重新定位为低空经济 / 城市空中交通的使能技术，并在 2026-05-23 整理中明确当前暂缓推进、先保留为主动感知使能技术方向。"
pubDate: 2026-05-15
...
### 0.1 2026-05-23 整理：当前优先级与边界

Paper C 仍然是有价值的主动感知方向，但当前不应与 G1、B、F-J1 抢近期主线资源。原因是它要同时证明 3DGS 主动感知算法、UAV 安全规划和交通系统价值，工作面比第一版预计更大。

当前建议把 Paper C 定位为 **P3 储备方向**：

| 项目 | 当前处理 |
|------|----------|
| 主贡献 | FIM-3DGS 主动视点选择 + UAV 安全约束 |
| 交通连接 | 只保留为巡检、应急、配送的使能技术，不先写成完整 TR-C 系统论文 |
| 必须补强 | 真实/公开 urban 3D 数据、强 NBV baseline、任务级指标、可复现仿真 |
| 暂缓内容 | 多 UAV 系统吞吐、低空经济政策叙事、完整 SUMO-AirSim 大系统 |
| 恢复条件 | G1 工具链稳定、F 场景平台可复用，或已有足够 3DGS/主动感知实验资产 |

如果后续要重启，第一篇应先按 **T-RO / T-ITS 方法论文标准** 打磨主动感知技术，确认技术指标站得住；只有当实验能证明它显著改善巡检/应急/配送的任务效率和安全指标时，再升级到 TR-C 系统论文。

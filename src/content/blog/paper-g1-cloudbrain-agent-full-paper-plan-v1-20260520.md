...
### 1.5 2026-05-23 整理：可投稿版本冻结清单

G1 的第一版投稿必须冻结三条 claim，避免变成低空平台说明书：

1. **Domain-grounded tool-use benchmark**：CloudBrain-Bench 不只测 JSON 格式，而是测低空交通工具链中的函数选择、参数 grounding、状态依赖、policy compliance 和多轮一致性。
2. **Verifier-guided repair**：低空交通任务中的安全错误、不可执行错误和模糊任务，要通过 LTL/STL verifier、route planner 和 simulator feedback 转化为结构化修复信号。
3. **Local-deployable agent implementation**：主实验必须能在本地开源模型上复现，API 模型只作为 teacher 或 upper bound。

第一篇必须完成的内容：

| 模块 | 冻结要求 |
|------|----------|
| LowAltitudeIR | 固定 schema、类型检查器、错误码和 JSON examples |
| Tools | 至少 6 个：airspace query、fleet status、assignment、route planner、LTL/STL verifier、scenario simulator / risk estimator |
| CloudBrain-Bench | dev / validation / test / stress split，覆盖 SAT、UNSAT、ambiguous、resource-limited、stress scenarios |
...
| Data layer | synthetic 主数据 + OSM/FAA/OD/SUMO 真实上下文字段，不把真实数据写成已部署系统 |

第一篇暂缓内容：

- 完整 MCP 产品化；
- 多 agent 协作作为主贡献；
- 把 LowAltitudeGPT 微调模型写成主方法；
- 真实 UAV 部署或实飞；
- VLA / world model / embodied AGI 主张。

这份冻结清单的作用是控制论文边界：G1 只证明“低空交通安全关键域中的可验证 LLM agent”成立，后续 G2/G3/G4 再分别处理微调、多智能体和 embodied 扩展。

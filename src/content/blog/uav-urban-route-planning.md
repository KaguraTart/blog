...
APF 的在线更新通常较轻量，适合实时避障；但若按前述定义直接计算最近障碍距离 $\rho(\mathbf{p})=\min_{obs}\|\mathbf{p}-\mathbf{p}_{obs}\|$，朴素实现每步通常至少需遍历障碍集合，即约为 $O(n_{obs})$。只有在已预计算距离场、ESDF 或栅格查询的前提下，单步查询才可近似视为 $O(1)$)。不过在 CBD 峡谷中它仍有一个致命弱点：**局部极小值**——当引力和斥力恰好平衡时，UAV 会卡在一个非目标点无法前进。改进方案包括随机扰动、谐波势场或与 RRT 结合的 PF-RRT 算法。
